...
export class CallResolver {
  private:
    ...
    bool check_arguments(const FunctionType* function_type, bool emit_errors = false) {
        ...
        for (std::size_t i = 0; i < required; ++i) {
            const auto* expected_type = resolver.resolve(function_type->parameters[i].type);
        ...
        if (function_type->variadic && parameter_count > 0) {
            const auto* element_type = resolver.resolve(function_type->parameters.back().type);
  ...
  public:
    explicit CallResolver(Context& context, SemaContext& sema, TypeResolver& resolver,
                          Visitor<void>& visitor, CallExpression& node)
